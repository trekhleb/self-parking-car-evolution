# Само-паркующаяся тачка в 500 строчек кода

> С помощью генетического алгоритма натренируем тачку выполнять самостоятельную парковку
...
**В ≈40-м поколении** автомобили по-немного начнут учиться парковке и будут все ближе и ближе подбираться к парковочному месту,
...
> Да-да, машины врезаются в другие машины по пути, и не точно становятся на парковочное место, но для них это всего-лишь 40-е поколение с момента создания мира, так что будьте снисходительны и дайте машинам подрасти :D
...
Вот как шаги преобразования будут выглядеть на сигмоидном графике.
...
Вы только взгляните! Бинарный геном выглядит таким загадочным. Но представьте себе, что эти `180` нулей и единиц определяют, как автомобиль ведет себя на стоянке! Это как если бы вы взломали чью-то ДНК и точно знаете, что означает каждый ген. Прикольно!
...
1. **СОЗДАНИЕ** - самое первое поколение автомобилей [не может появиться из ничего](https://en.wikipedia.org/wiki/Laws_of_thermodynamics), поэтому мы сгенерируем набор случайных геномов (набор двоичных массивов длиной `180`). Например, мы можем создать `~1000` машин. Чем больше количество машин (население), тем выше шансы найти оптимальное решение (и найти его быстрее).
...
Использовать эту функцию довольно просто. Допустим, вы очень любите бананы и хотите есть их чаще, чем клубнику. Вы можете вызвать функцию `const fruit = weightedRandom(['banana', 'strawberry'], [9, 1])`, и в `≈9` случаях из `10` переменная `fruit` будет равна `banana`, и только в `≈1` из `10` случаем она будет равна `strawberry`.
